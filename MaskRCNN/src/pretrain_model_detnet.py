...
class Detnet_FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)     # 4 stride
        x = self.layer2(x)    # 8 stride
        x = self.layer3(x)    # 16 stride
        x = self.detLayer1(x) # 16 stride
        x = self.detLayer2(x) # 16 stride
...
def freeze(pretrained_model, model):
...
def detnet59_fpn(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model = freeze(pretrained_model, model)
    return model
